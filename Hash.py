...
class Hash:
    ...
    def insert(self, string):
        rec = Record(string)
        self.w_block.write(abs(hash(rec.cpf)//10**6) *
                           self.w_block.max_size*self.w_block.record_size, rec)
        self.w_block.persist(abs(hash(rec.cpf)//10**6) *
                             self.w_block.max_size*self.w_block.record_size)
        for i in self.indexes:
            self.indexes[i].write(getattr(rec, i)+" "+str(abs(hash(rec.cpf)//10**6)
                                                          * self.w_block.max_size*self.w_block.record_size)+"\n")
