...
const Record* Heap::sel(const char* cpf) {
  ...
  do {
    for(int i = 0; i < this->blockg->count(); i++) {  // For i = 0 to i = number of reading block's records
      ...
      for (int j = 0; j < sizeof(record->cpf); j++) {
        if (record->cpf[j] != cpf[j]) {               // Verify if record's cpf is different from query's cpf
          found = 0;
          break;
        }
      }
      if (found) {                                    // If found record with query's cpf return the record
